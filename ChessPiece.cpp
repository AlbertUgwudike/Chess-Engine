...
void ChessPiece::reportInvalidMove(Position position) {
  std::cout << this << " cannot move to position " << position.str() << std::endl;
}

const char *ChessPiece::str() const {
  switch (type()) {
    case tPawn: return "Pawn";
    case tRook: return "Rook";
    case tKnight: return "Knight";
    case tBishop: return "Bishop";
    case tKing: return "King";
    case tQueen: return "Queen";
  }
  return "NULL";
}

std::ostream& operator << (std::ostream& os, ChessPiece *p) {
  return os << (p ? p->str() : "Empty");
}
...
bool Rook::canMove(const ChessBoard *board) const {
  ...
  for(Move move : { basicMoves[U], basicMoves[D], basicMoves[L], basicMoves[R] }) {
...
bool Bishop::verifyMove(const ChessBoard *board, Position destination) const {
  ...
  for(Position pos = next; pos != destination; pos = pos.go(basicMoves[direction]))
    if (board->getPiece(pos)) return false;
...
bool Bishop::canMove(const ChessBoard *board) const {
  // attempt four possible rook moves
  for(Move move : { basicMoves[UL], basicMoves[DL], basicMoves[DL], basicMoves[DR] }) {
    if (!position().canGo(move)) continue;
    if (verifyMove(board, position().go(move))) return true;
  }
...
bool King::verifyMove(const ChessBoard *board, Position destination) const {
  ...
  if (moveCount() == 0 && rankDiff == 0 && abs(fileDiff) == 2) {
    ...
    // check spaces between are unmarked and empty
    const Move move = basicMoves[fileDiff > 0 ? L : R];
    for (Position pos = position(); pos != rook->position(); pos = pos.go(move)) {
      ChessPiece *piece = board->getPiece(pos);
      if (piece && (piece->type() == tRook || piece->type() == tKing)) continue;
      if (piece || board->isMarked(pos)) return false;
    }
...
bool Pawn::verifyMove(const ChessBoard *board, Position destination) const {
  ...
  if (color() && rankDiff < 0 || !color() && rankDiff > 0)
    return false;
  
  // double move
  if (fileDiff == 0 && abs(rankDiff) == 2 && moveCount() == 0) {
    const Move move = basicMoves[rankDiff > 0 ? U : D];
    Position fst = position().go(move); Position snd = fst.go(move);
  ...
  // en passant
    ...
    if (!(adjacent &&
          ...
          adjacent->type() == tPawn &&
          adjacent->moveCount() == 1 &&
          adjacent->position().rank() == color() ? 3 : 4)) return false;
  }
